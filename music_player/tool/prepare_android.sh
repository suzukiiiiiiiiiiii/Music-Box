...
#
# tool/setup.sh の手順のうち、手作業で直すよう書かれていた部分
# (minSdk / compileSdk) までまとめて済ませます。CI からもこれを呼びます。
# lib/ と pubspec.yaml は git から戻すので、変更が消えることはありません。
set -euo pipefail
...
cp android_setup/AndroidManifest.xml android/app/src/main/AndroidManifest.xml
